...
export function IngredientRecipe({
  ...
  return (
    <article
      className={cn(
        "relative overflow-hidden rounded-[1.75rem] border border-line bg-[#fffdf7] shadow-soft",
        className,
      )}
    >
      <div
      ...
      <header className="border-b border-dashed border-line px-6 pb-6 pt-8 sm:px-10">
        ...
        <dl className="mt-5 flex flex-wrap gap-x-8 gap-y-2 text-sm">
          <div>
            ...
            <dd className="text-muted">{recipeYield.replace(/^Yield:\s*/, "")}</dd>
          </div>
          <div>
          ...
          <div>
            ...
            <dd className="text-muted">In-house school kitchen</dd>
          </div>
        </dl>
      </header>

      <div className="grid gap-0 lg:grid-cols-2">
        <section className="px-6 py-7 sm:px-10" aria-labelledby="recipe-ingredients-heading">
          <h4
        ...
        <section
          className="border-t border-dashed border-line bg-white/70 px-6 py-7 sm:px-10 lg:border-l lg:border-t-0"
          ...
          <ol className="mt-5 space-y-4">
            {recipeMethod.map((step, index) => (
              <li key={step} className="flex gap-4">
                <span
                  className="flex h-8 w-8 shrink-0 items-center justify-center rounded-full bg-green/15 text-sm font-bold text-green-deep"
                  aria-hidden="true"
                >
                  {index + 1}
                </span>
                <p className="pt-1 text-base leading-relaxed text-muted">{step}</p>
              </li>
            ))}
          </ol>
        </section>
